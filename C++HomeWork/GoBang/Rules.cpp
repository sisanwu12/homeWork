...
int IsWin(vector<Piece>& count,bool BorW,Board* board){
	...
	if (!BorW) {
		if (checkWin(xCount) || checkWin(yCount) || checkWin(xyCount) || checkWin(yxCount)) return 1;
	}
	else {
		if (Forbidden(xCount, yCount, xyCount, yxCount)) return -1;
		if (checkWin(xCount) || checkWin(yCount) || checkWin(xyCount) || checkWin(yxCount)) return 2;
	}
	return 0;
}
